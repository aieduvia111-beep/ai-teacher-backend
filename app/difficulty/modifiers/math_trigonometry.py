# -*- coding: utf-8 -*-
"""Adapter dla trygonometrii - opakowuje ISTNIEJACY kod w
app/math_verify.py (classify_trig_difficulty / validate_trig_difficulty),
dokladnie ten sam wzorzec co math_quadratic.py/math_sequences.py (Czesc 12
audytu: "domain-specific modifier/adapter, nie kopiuj jego logiki do
kolejnych tematow").
...
identyczne jak wprost wywolanie validate_trig_difficulty."""
from typing import List, Optional

from . import DomainModifier
from ...math_verify import classify_trig_difficulty, validate_trig_difficulty
from ...level_config import TRIG_DIFFICULTY_TIERS
from ..calibration import level_adjusted_shift, TRIG_BASELINE_LEVEL, TRIG_MAX_SHIFT

_TIER_ORDER = list(TRIG_DIFFICULTY_TIERS.keys())  # ["1", "2-3", "4-5"]
...
def _trig_level_shift(level: str) -> int:
    return level_adjusted_shift(level, TRIG_BASELINE_LEVEL, TRIG_MAX_SHIFT)
...
class TrigonometryModifier(DomainModifier):
    ...
    def evaluate(self, question_text: str, option_texts: Optional[List[str]],
        ...
        if not level or _trig_level_shift(level) == 0:
            return validate_trig_difficulty(question_text, requested_difficulty_word)

        detected_tier = classify_trig_difficulty(question_text)
        if detected_tier is None:
            return {"status": "not_trig"}
